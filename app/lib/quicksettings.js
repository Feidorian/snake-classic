...
(function () {
	...
	var QuickSettings = {
		...
		_init: function (x, y, title, parent, secureStorage) {
			if (!cssInjected) {
				injectCSS();
			}
			this._bindHandlers();
		...
		saveInLocalStorage: function (name) {
			this._secureStorageName = name;
			this._readFromSecureStorage(name);
			return this;
		},
		...
		clearLocalStorage: function (name) {
			this._secureStorage.removeItem(name);
			return this;
		},

		_saveInLocalStorage: function (name) {
			this._secureStorage.setItem(name, this.getValuesAsJSON(true));
		},

		_readFromSecureStorage: function (name) {
			var str = this._secureStorage.getItem(name);
			if (str) {
		...
		_callGCH: function (title) {
			if (this._secureStorageName) {
				this._saveInLocalStorage(this._secureStorageName);
			}
		...
		addBoolean: function (title, value, callback) {
			...
			var id = title.replace(/\s/g, ''); //;getNextID();
		...
		addButton: function (title, callback) {
			var container = this._createContainer();
			var id = title.replace(/\s/g, ''); //getNextID();
			var button = createInput('button', id, 'qs_button', container);
			button.value = title;
		...
		addDate: function (title, date, callback) {
			...
			var id = title.replace(/\s/g, ''); //getNextID()
			var dateInput = createInput('date', id, 'qs_text_input', container);
		...
		_addText: function (type, title, text, callback) {
			...
			var id = title.replace(/\s/g, ''); //getNextID()
			if (type === 'textarea') {
				textInput = createElement('textarea', id, 'qs_textarea', container);
				textInput.rows = 5;
			} else {
				textInput = createInput(type, id, 'qs_text_input', container);
			}
		...
		addTime: function (title, time, callback) {
			...
			var label = createLabel('<b>' + title + '</b>', container);
			var id = title.replace(/\s/g, ''); //getNextID()
			var timeInput = createInput('time', id, 'qs_text_input', container);
